  ...
  return (
    <div className="notes-container">
      ...
      <div className="notes-section">
        {notes.map((note, idx) => (
          <div key={idx} style={{display:'flex', width:'70vw', justifyContent:'flex-start', height:'6rem', marginLeft:'3rem', flexWrap:'wrap'}}>
            <div>
              <p style={{margin:'0'}}>{note.date}</p>
              <p style={{margin:'0'}}>{note.time}</p> 
            </div>
            <div style={{marginLeft:'6rem', width:'80%'}}>{note.text}</div>
          </div>
        ))}
      </div>
      <div>
