  ...
  return (
    <div className="notes-container">
      ...
      <div className="notes-section">
        {notes.map((note, idx) => (
          <div key={idx} style={{display:'flex', width:'70vw', justifyContent:'flex-start', alignItems:'center', height:'6rem', marginLeft:'3rem'}}>
            <div>
              <p>{note.date}</p>
              <p>{note.time}</p> 
            </div>
            <div style={{marginLeft:'6rem'}}>{note.text}</div>
          </div>
        ))}
      </div>
      <div>
